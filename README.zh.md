...
远端 ComfyUI 工作流 DSH 插件（开源）：经远端 ComfyUI 做任务提交、状态查询、等待与中断；工作流模板随包发布。零运行时依赖。

## 状态

W3 已落地：`comfy_submit` / `comfy_status` / `comfy_wait` / `comfy_cancel` / `comfy_templates` 五工具已实现、有测试、全绿。

## 上游

ComfyUI 是 comfyanonymous 的开源项目。本包仅为其 DSH 接入客户端：直调 ComfyUI 原生 HTTP 接口（`/prompt`、`/history`、`/queue`、`/upload`、`/view`），随包发布 API 格式工作流模板与 `templates/manifest.json` 登记表，未内嵌上游代码。
...
基地址缺失即大声失败并给出路；绝不静默降级。鉴权 token 只来自环境变量 `COMFYUI_AUTH_TOKEN`，只进 `Authorization` 请求头——永不进日志、收据与产物。

工作区解析：`config.workspace` > 环境变量 `WWRS_WORKSPACE` > 向上探测 `.wwrs/workspace.json` > 大声失败。所有路径（产物、资产）必须用工作区相对路径；绝对路径或 `..` 逃逸一律拒收。任务收据落在工作区内 `.wwrs/comfy-tasks/<taskId>.json`（不含凭据）。
...
| `comfy_wait`      | `taskId`、`until?`（`terminal`\|`submitted`）、`timeoutMs?` | 分段等待，永不 408：到点返回 `renewable=true` 由调用方循环续期；terminal 成功即下载首个产物到 `outputPath`；`exec.signal` 只中断等待，不动远端任务 |
